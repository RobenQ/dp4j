 ...
 * 是用户调用getInstance方法时再将实例变量的对象new出来。但这是线程安全的，因为加
 * synchronized锁了，同时只有一个线程能进入该方法执行，保证了单例性。
...
public class ExThree {
    ...
    private static ExThree singleton;

    private ExThree() {
    }

    public static synchronized ExThree getInstance(){
        if(singleton==null)
            singleton = new ExThree();
        return singleton;
    }

    public static void main(String[] args) {
        ExThree a = ExThree.getInstance();
        ExThree b = ExThree.getInstance();
